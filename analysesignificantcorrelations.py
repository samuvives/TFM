...
# I need to normalize by the number of factors, and 
# also normalize the p-value inside the run with benjamini-hochberg
...
for factordir in factordirs:
    ...
    pvaluetable["p-value_adj"] = false_discovery_control(pvaluetable["p-value"])
    factorstable = pd.merge(correlationtable, pvaluetable, on="FactorvsFactor")
    print(factorstable)

    numfactors = int(factordir.replace("K", ""))
    numbersignificantcorrelations = (factorstable["p-value_adj"] < 0.05).sum()
    numbersignificantcorrelations = numbersignificantcorrelations / numfactors
...
ax.set_title(f"Number of significant correlations across the runs,\n normalized, approach={approach}")
plt.tight_layout()
