# poner todos los postanalysis importados aqui
import os
import pandas as pd
...
def corfactorsgv(model_factors, gv_names, df_combined):
...
for k_dir in k_folders:
    ...
    # Añadimos un prefijo "Model_" a todas las columnas que vienen de MOFA para que no choquen
    df_Z_raw.columns = [f"Model_{c}" for c in df_Z_raw.columns]

    # hacerlo mejor con las gv
    df_gv.columns = [f"GV_{c}" for c in df_gv.columns]
    ...
    # RELATIONS WITH GUIDING VARIABLES 
    corfactorsgv(model_factors, gv_names, df_combined)
